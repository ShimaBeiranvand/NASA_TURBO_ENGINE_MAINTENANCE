...
def load_data(index="FD004"):
    if type(index) == str:
        ...
        if index == "FD004":
            ...
            print("Conditions: SIX")
            print("Fault Modes: TWO (HPC Degradation, Fan Degradation)\n")    
        train_set = np.loadtxt(f"/content/drive/MyDrive/NASA_data_file/unpacked_data/train_{index}.txt")
        test_set  = np.loadtxt(f"/content/drive/MyDrive/NASA_data_file/unpacked_data/test_{index}.txt")    
        ...
        labels = np.loadtxt(f"/content/drive/MyDrive/NASA_data_file/unpacked_data/RUL_{index}.txt")    
...
def run_to_failure_aux(df, lifetime, unit_number):
    assert lifetime <= df.shape[0], "Lifetime exceeds the number of rows in the DataFrame"
    ...
    sensors = df.filter(regex='sensor')
    num_features = sensors.iloc[:lifetime].agg(['min', 'max', 'mean', 'std']).unstack().reset_index()
    num_features['feature'] = num_features['level_0'] + '_' + num_features['level_1']
...
# load the data now
dataset = generate_run_to_failure(train_set,health_censor_aug=train_set.unit_number.nunique() * 3)
...
def leave_one_out(target='run-to-failure',health_censor_aug=1000, seed=123,input_fn=None, output_fn=None):
...
plt.show()
val[0][1].broken.hist()
print('len of validation: ',len(val))
...
def main(train_set, test_set, n_estimators=100, learning_rate=0.1, registered_model_name=None, model_path=None):
    ...
    # CREATING X-TRAIN , X-TEST , Y-TRAIN , Y-TEST
    # assigning X and y to train_df and test_df   
    # Extracting the label column
    y_train = train_df.pop("lifetime")    
    # convert the dataframe values to array
    X_train = train_df.values    
    # Extracting the label column
    y_test = test_df.pop("lifetime") 
    # convert the dataframe values to array
    ...
    # SAVING THE MODEL
    # Saving with mlflow - leave this section unchanged
    with tempfile.TemporaryDirectory() as td:
        ...
        for file_name in os.listdir(tmp_output_dir):
            ...
            shutil.copy2(src=os.path.join(tmp_output_dir, file_name), dst=os.path.join('/content/drive/MyDrive/NASA_code/outputs/conda.yaml'))   # "./outputs"
    ...
    mlflow.sklearn.save_model(
        sk_model=model,
        path=os.path.join('/content/drive/MyDrive/NASA_code/trained_model'),
    )    
